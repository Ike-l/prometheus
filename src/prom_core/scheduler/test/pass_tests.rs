...
fn single_res() {
    ...
    scheduler.insert_system(0., single_res_system);
    scheduler.insert_resource(Res1(1));
...
fn multiple_res() {
    let mut scheduler = create_scheduler();
    scheduler.insert_system(0., many_res_system1);
    scheduler.insert_system(0., many_res_system2);
    scheduler.insert_resource(Res1(1));
    scheduler.insert_resource(Res2(3));
...
fn mut_res() {
    let mut scheduler = create_scheduler();
    scheduler.insert_system(0., single_mut_system);
    scheduler.insert_system(0.1, check_mutate_phase_single_mut_system);
    scheduler.insert_system(1., check_mutate_tick_single_mut_system);
    scheduler.insert_resource(ResMut1(1));
...
fn access_mut_over_phases() {
    let mut scheduler = create_scheduler();
    scheduler.insert_system(0., access_mut_over_phases_system1);
    scheduler.insert_system(0.1, access_mut_over_phases_system2);
    scheduler.insert_resource(ResMut1(1));
...
fn get_many_ref_world() {
    let mut scheduler = create_scheduler();
    scheduler.insert_system(0., get_ref_world_system1);
    scheduler.insert_system(0., get_ref_world_system2);
...
fn many_reader_writer_works() {
    let mut scheduler = create_scheduler();
    scheduler.insert_system(0., many_event_writer_system1);
...
fn float_precision_order_test() {
    let mut scheduler = create_scheduler();
    scheduler.insert_system(0.5000000000000001 + 2. * f64::EPSILON, float_precision_test_system6);
    scheduler.insert_system(0.49999999999999994, float_precision_test_system3);
    scheduler.insert_system(0.5000000000000001, float_precision_test_system4);
    scheduler.insert_system(f64::MIN_POSITIVE / 2.0, float_precision_test_system2);
    scheduler.insert_system(0.5000000000000001 + f64::EPSILON, float_precision_test_system5);
    scheduler.insert_system(0., float_precision_test_system1);
    scheduler.insert_resource(1);

    run_scheduler_start(scheduler);
}

fn insert_same_type_system(res1: Res<i32>, res2: Res<i32>) {
    assert_eq!(*res1, 2);
    assert_eq!(*res2, 2);
}
...
fn insert_same_type() {
    let mut scheduler = create_scheduler();
    scheduler.insert_system(0., insert_same_type_system);
    scheduler.insert_resource(1 as i32);
    scheduler.insert_resource(2 as i32);
